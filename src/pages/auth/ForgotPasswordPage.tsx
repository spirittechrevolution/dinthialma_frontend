...
const STEPS = [{ label: 'Téléphone' }, { label: 'Vérification' }, { label: 'Nouveau MDP' }]
...
function AuthHeader({ step }: { step: number }) {
  const subtitles = ['Entrez votre numéro', 'Confirmez par SMS', 'Choisissez un nouveau mot de passe']
  return (
    <div className="relative bg-gradient-to-br from-[#0d1f0f] via-primary-800 to-primary-600 pt-12 pb-20 px-6 text-center flex-shrink-0">
      <div className="flex flex-col items-center gap-3 mb-4">
        ...
        <div>
          <p className="text-white text-lg font-extrabold leading-none">Mot de passe oublié</p>
...
export function ForgotPasswordPage() {
  ...
  const form1 = useForm<Step1Data>({ resolver: zodResolver(step1Schema), defaultValues: { phone: '+221' } })
  const form2 = useForm<Step2Data>({ resolver: zodResolver(step2Schema) })
  const form3 = useForm<Step3Data>({ resolver: zodResolver(step3Schema) })

  const onStep1 = async ({ phone: p }: Step1Data) => {
    setError(null)
    try { await authService.sendForgotPasswordOtp({ phone: p }) } catch { /* anti-enumération */ }
    setPhone(p)
    setStep(2)
  }
  ...
  const onStep3 = async ({ newPassword }: Step3Data) => {
    ...
    } catch { setError('Erreur lors de la réinitialisation. Recommencez.') }
  }
      ...
      {step === 3 && (
        <form onSubmit={form3.handleSubmit(onStep3)} className="space-y-4">
          <Input label="Nouveau mot de passe" type="password" placeholder="••••••••" icon={<Lock size={16}/>}
            error={form3.formState.errors.newPassword?.message} {...form3.register('newPassword')}/>
          <Input label="Confirmer le mot de passe" type="password" placeholder="••••••••" icon={<Lock size={16}/>}
    ...
    <>
      ...
      <div className="hidden lg:flex min-h-screen">
        <div className="w-1/2 bg-[#0d1f0f] flex flex-col justify-between px-12 py-10">
          ...
          <div>
            <h2 className="text-white text-4xl font-bold leading-tight mb-4">Récupérez</h2>
            <h2 className="text-primary-400 text-4xl font-bold leading-tight mb-6">votre accès.</h2>
            <p className="text-neutral-400 text-base leading-relaxed">
              Réinitialisez votre mot de passe en quelques<br/>secondes grâce à un code SMS.
            </p>
          </div>
        ...
        <div className="flex-1 flex items-center justify-center bg-neutral-50 px-8">
          <div className="w-full max-w-md">
            <div className="mb-6">
              ...
              <p className="text-neutral-500 mt-1">{['Entrez votre numéro de téléphone', 'Confirmez votre identité par SMS', 'Choisissez un nouveau mot de passe'][step - 1]}</p>
            </div>
      ...
      <div className="lg:hidden min-h-screen flex flex-col bg-neutral-50">
        ...
        <p className="text-center text-xs text-neutral-400 py-4">© 2026 Dinthialma</p>
      </div>
    </>
  )
}
